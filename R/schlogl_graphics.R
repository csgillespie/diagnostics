...
d_ms_eb = readRDS(file="data/N_10000_schlogl_err_sims_large_eb.RData")
out_eb = readRDS(file="data/N_10000_schlogl_err_lna_sims_large_eb.RData")
...
## Top 30 residuals
g0 = ggplot(dd) + 
  geom_point(data=dd[abs(dd$res) < 6, ], aes(V2, V1), col=NA, size=0.7, pch=21, bg=3) +
  geom_point(data=dd[abs(dd$res) > 6, ], aes(V2, V1), col="black", size=1, pch=21, bg=4) +
  annotation_logticks() + 
  xlab(expression(c[3])) + 
  ylab(expression(c[4])) 
g0
g = g0 + scale_x_continuous(trans="log10", 
  ...
  annotate("text", x=10^{-4}, 10, label="(b)", size=4, vjust = -0.5)
g
which.max(dd$res)
###################################################################
...
g2 = g1 + scale_x_continuous(trans="log10", 
  ...
  guides(lty=FALSE) + 
  annotate("text", x=10^{-2}, 200, label="(c)", size=4)
g2 = g2 +   annotation_logticks(sides="b")
g2
################################################################
...
# 
# 
# d_ms_sub = data.frame(d_ms_eb)#[d_ms$sim_no < 51,]
# g6 = ggplot(d_ms_sub) + 
#   geom_step(aes(Time, X, group=sim_no), alpha=0.15) + 
#   xlab("Time") + 
#   ylab("Population") #+ 
# #  ylim(c(0, 800))
# g6
# g7 = g6 + geom_line(data=data.frame(out_eb), aes(Time, X), col=4)
# g8 = g7 + annotate("text", x=1.2, y = 250, label="LNA approximation", col=4, size=4) + 
#   annotate("text", x=0, 800, label="(d)", size=4)
# g8
